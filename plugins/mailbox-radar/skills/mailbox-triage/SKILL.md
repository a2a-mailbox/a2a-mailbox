...
```
node S/claim.mjs <訊息檔絕對路徑>
```

**給絕對路徑**：腳本會自己判斷那封屬於哪個交換區。只有檔名、沒有路徑時，額外交換區的訊息要多帶 `--exchange <交換區名稱>`，
否則兩個交換區的同名檔會搶同一張票。腳本回錯誤（exit 2）代表參數沒看懂、**沒有認領到**，修好參數重跑，不要當成認領成功往下走。
...
- 記帳**用腳本，不要自己編輯檔案**（散文指示會被忘記、寫錯路徑、寫錯格式，
  三種錯法當下都沒有跡象；2026-09-10 實際造成一臺機器兩個月沒記帳、雷達長期虛報）：

  ```
  node S/markread.mjs --note "已回覆 tier0" <訊息檔名>
  ```

  註記照處理結果填：`已回覆 tier0`／`已回覆待回報 tier1`／`待本人決定 tier2`。
  回報裡的 `added` 有值才算記成功；`skipped` 代表這封先前已經記過了。
  雷達不會自動記帳——未讀數的語意是「人還沒處理」，所以只有這一步（真的處理完了）才記。
  訊息屬於額外交換區時（閘門判定結果的 `exchangeId` 有值），記帳要多帶 `--exchange <那個值>`。
  閘門與認領鎖會自己從訊息檔路徑判斷是哪個交換區，不用另外指定。
