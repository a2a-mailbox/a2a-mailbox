...
**兩臺電腦互傳的建法**：用其中一個 Google 帳號建一個資料夾當交換區，裡面建 `收件匣-<A 機代稱>`、`收件匣-<B 機代稱>`、`公告板` 三個子資料夾，分享給另一臺登入的 Google 帳號（權限：編輯者），再到 Drive 網頁加捷徑到「我的雲端硬碟」。兩臺如果寫檔用的是同一個 Google 帳號，Drive 記的檔案擁有者會是同一個 email，通訊錄就寫成一列、兩個代稱。
