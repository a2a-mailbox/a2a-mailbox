...
| 更新之後，開著的對話收不到新功能的通知，或 Mac 的離席通知行為沒變 | 通知器與桌鈴是常駐的背景行程，`claude plugin update` 不會重啟它們，它們會繼續跑舊版的程式。**更新之後，把開著的對話全部重開。** 0.7.1 起雷達會在開對話或 Claude 用工具時比對背景行程的版本，不一樣就換掉；但做比對的那段程式本身，在已經開著的對話裡也不一定馬上換成新版。實測過同一臺機器上，有的對話更新後一分鐘內就換了，有的過了幾十分鐘還是舊的，沒辦法預期。重開的對話一定是新版，所以重開是唯一確定的做法，自動換版只是沒重開時的補救 |
