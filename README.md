...
| Mac 更新之後，離席通知的行為沒變 | 桌鈴是常駐的背景行程，`claude plugin update` 不會重啟它，它會繼續跑舊版的程式。更新後在終端機跑 `kill $(cat ~/.claude/plugins/data/mailbox-radar/deskbell.pid)`，下一次開對話或 Claude 用工具時就會用新版重新帶起來 |
